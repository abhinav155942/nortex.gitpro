...
Welcome! This guide provides all the details you need to contribute effectively to the project. Thank you for helping us make **nortex.diy** a better tool for developers worldwide. 💡
...
- Clone the repository:
  ```bash
  git clone https://github.com/stackblitz-labs/nortex.diy.git
  ```
...
```bash
docker run -p 5173:5173 --env-file .env.local nortex-ai:development
```
...
```bash
docker build . --target nortex-ai-production
```
...
```bash
docker run -p 5173:5173 --env-file .env.local nortex-ai:production
```
