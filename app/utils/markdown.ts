  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__nortexThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__nortexArtifact__', '__nortexThought__', '__nortexQuickAction', '__nortexSelectedElement__'],

      // ['className', '__nortexThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__nortexArtifact__', '__nortexThought__', '__nortexQuickAction'],
    ],
  },
  strip: [],
};
