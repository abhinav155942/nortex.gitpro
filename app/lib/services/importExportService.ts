...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          nortex_user_profile: this._safeGetItem('nortex_user_profile'),
          nortex_settings: this._safeGetItem('nortex_settings'),
          nortex_profile: this._safeGetItem('nortex_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('nortex_viewed_features'),
          developer_mode: this._safeGetItem('nortex_developer_mode'),
        ...
        ui: {
          // Tab configuration
          nortex_tab_configuration: this._safeGetItem('nortex_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('nortex_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('nortex_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          nortex_read_logs: this._safeGetItem('nortex_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('nortex_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('nortex_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.nortex_tab_configuration) {
        try {
          this._safeSetItem('nortex_tab_configuration', data.ui.nortex_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'nortex_acknowledged_debug_issues',
        'nortex_acknowledged_connection_issue',
        'error_logs',
        'nortex_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('nortex_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
