import type { ActionType, NortexAction, NortexActionData, FileAction, ShellAction, SupabaseAction } from '~/types/actions';
import type { NortexArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<nortexArtifact';
const ARTIFACT_TAG_CLOSE = '</nortexArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<nortexAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</nortexAction>';
const NORTEX_QUICK_ACTIONS_OPEN = '<nortex-quick-actions>';
const NORTEX_QUICK_ACTIONS_CLOSE = '</nortex-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends NortexArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: NortexArtifactData;
  currentAction: NortexActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(NORTEX_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(NORTEX_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + NORTEX_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <nortex-quick-action ...>label</nortex-quick-action> inside
          const quickActionRegex = /<nortex-quick-action([^>]*)>([\s\S]*?)<\/nortex-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + NORTEX_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as NortexAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as NortexAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__nortexQuickAction__"',
    'data-nortex-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class=\"__nortexQuickAction__\" data-nortex-quick-action=\"true\">${buttons.join('')}</div>`;
}
