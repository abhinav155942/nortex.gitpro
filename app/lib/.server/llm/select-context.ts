...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyNortexActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyNortexActions(content);

      content = content.replace(/<div class=\\"__nortexThought__\\">.*?<\/div>/s, '');
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`Nortex failed to select files`);
  }
