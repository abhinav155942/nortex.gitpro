...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      nortexVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.nortexVersion) {
      body += `- nortex: ${data.environmentInfo.nortexVersion}\n`;
    }
  ...
  body += '---\n*Submitted via nortex bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/nortex';
