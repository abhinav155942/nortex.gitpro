...
We have also launched an experimental agent called the "nortex.diy Expert" that can answer common questions about nortex.diy. Find it here on the [oTTomator Live Agent Studio](https://studio.ottomator.ai/).

nortex.diy was originally started by [Cole Medin](https://www.youtube.com/@ColeMedin) but has quickly grown into a massive community effort to build the BEST open source AI coding assistant!
...
[Join the nortex.diy community here, in the oTTomator Think Tank!](https://thinktank.ottomator.ai)

## Project management

Nortex.diy is a community effort! Still, the core team of contributors aims at organizing the project in way that allows
...
Let's get you up and running with the stable version of Nortex.DIY!

## Quick Installation

[![Download Latest Release](https://img.shields.io/github/v/release/stackblitz-labs/nortex.diy?label=Download%20Nortex&sort=semver)](https://github.com/stackblitz-labs/nortex.diy/releases/latest) ← Click here to go to the latest release version!
...
You have two options for running Nortex.DIY: directly on your machine or using Docker.
...
2. **Build an Image**
   ...
   # ≈ docker build -t nortex-ai:development -t nortex-ai:latest --target development .

   # Production image (self-contained build artifacts)
   pnpm run dockerbuild:prod
   # ≈ docker build -t nortex-ai:production -t nortex-ai:latest --target nortex-ai-production .
   ```
...
3. **Run the Container**
   ...
   docker run --rm -p 5173:5173 --env-file .env.local nortex-ai:latest
   ```
...
For users who prefer a native desktop experience, nortex.diy is also available as an Electron desktop application:

1. **Download the Desktop App**:
   - Visit the [latest release](https://github.com/stackblitz-labs/nortex.diy/releases/latest)
...
### Option 4: Cloud Deployment (Vercel, Netlify, Cloudflare)

Nortex is 100% ready for deployment to major cloud platforms.

#### Vercel
1. Install [Vercel CLI](https://vercel.com/download): `npm install -g vercel`
2. Run `vercel` in the project root
3. Follow the prompts to deploy

#### Netlify
1. Install [Netlify CLI](https://docs.netlify.com/cli/get-started/): `npm install -g netlify-cli`
2. Run `netlify deploy --build`
3. Follow the prompts to deploy

#### Cloudflare Pages
1. Run `pnpm run deploy`
2. This will build the project and use Wrangler to deploy to Cloudflare Pages

## Configuring API Keys and Providers

Nortex.diy features a modern, intuitive settings interface for managing AI providers and API keys. The settings are organized into dedicated panels for easy navigation and configuration.
...
Nortex.diy supports both methods for maximum flexibility:
...
1. **Clone the Repository**:
   ...
   ```bash
   git clone -b stable https://github.com/stackblitz-labs/nortex.diy.git
   ```
...
>     - Explanation: The System Prompt currently implemented in nortex.diy cant cover the best performance for all providers and models out there. So it works better with some models, then other, even if the models itself are perfect for >programming
